...
static void end_sr_on_error(struct speech_rec *sr, int errcode)
{
	...
	if (sr->session_id) {
		if (sr->notif.on_speech_end)
			sr->notif.on_speech_end(errcode, sr->notif.udata);
...
static void sr_continue(struct speech_rec *sr) {
	int errcode = sr_start_listening(sr, TRUE);
	if (errcode) {
		sr_dbg("speech recognizer restart failed\n");
		if (sr->notif.on_speech_end)
			sr->notif.on_speech_end(errcode, sr->notif.udata);
	}
}
...
int sr_start_listening(struct speech_rec *sr, BOOL restart)
{
	...
	if (!restart && sr->notif.on_speech_begin)
		sr->notif.on_speech_begin(sr->notif.udata);
